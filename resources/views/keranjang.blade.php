...
<body>
		...
		<div class="container">
			...
			<div class="table-responsive cart_info">
				<table class="table table-condensed">
					...
					<tbody>
					<?php $total= 0; $id_keranjang = 0; if($id != 0) { ?>
                        @foreach($keranjang as $krj)
						<tr>
							<td class="cart_product">
								<a href=""><img src="{{ asset('storage/data_file/'.$krj->gambar) }}" alt=""></a>
							</td>
							<td class="cart_description">
								<h4><a href="">{{$krj -> nama_produk}}</a></h4>
							</td>
							<td class="cart_price">
								<p><?php echo "Rp ".number_format($krj -> harga, 0,',','.'); ?></p>
							</td>
							<td class="cart_quantity">
								{{$krj -> jumlah}}
							</td>
							<td class="cart_total">
								<p class="cart_total_price"><?php echo "Rp ".number_format($krj->harga * $krj->jumlah, 0,',','.'); ?></p>
							</td>
						</tr>
						<?php $total += ($krj->jumlah * $krj->harga); $id_keranjang = $krj->id_keranjang ?>
                        @endforeach
						<?php } else{}?>
					</tbody>
				</table>
	...
	<section id="do_action">
		<div class="container">
			<div class="heading">
			<?php if($id != 0) { ?>
					<?php if ($id_keranjang != 0) { ?>
						<h3>Silahkan checkout pesanan anda!</h3>
						<p>Pastikan bahwa produk yang anda pesan sudah sesuai dengan apa yang anda inginkan yaa, terima kasih :)</p>
					<?php }else{ ?>
						<h3>Anda belum memesan barang nih !</h3>
						<p>Untuk memesan barang, silahkan pesan barang dulu yaa :)</p><?php } ?>
				<?php }else{?>
					<h3>Anda belum login !</h3>
				<p>Untuk memesan barang, silahkan login dulu yaa :)</p> <?php } ?>
			</div>
            <br><br>
			<div class="row">
            <div class="col-sm-6">
				...
				<div class="col-sm-6">
					<div class="total_area">
						<ul>
						<?php if($id != 0) { ?>
							<li>Total <span><?php echo "Rp ".number_format($total, 0,',','.'); ?></span></li>
							<?php } else{ echo "<li>Total <span>Rp 0</span></li>";}?>
						</ul>
